...
# Preload NVIDIA CUDA/cuDNN DLLs for ONNX Runtime
if sys.platform == "win32":
    try:
        import onnxruntime
        onnxruntime.preload_dlls()
        print("[startup] NVIDIA CUDA DLLs preloaded", flush=True)
    except Exception as exc:
        print(f"[startup] CUDA DLL preload failed: {exc}", flush=True)
