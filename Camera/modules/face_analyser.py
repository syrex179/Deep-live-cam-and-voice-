...
def _live_detection_size() -> tuple[int, int]:
    """Return the detector input size for the current live mode.

    The swap model still receives the original processed frame.  Reducing only
    RetinaFace's detection raster in Performance mode lowers its CUDA burst
    size while retaining fresh five-point landmarks on every frame.
    """
...
def detect_one_face_fast(frame: Frame) -> Any:
    ...
    bboxes, kpss = fa.det_model.detect(
        frame, input_size=_live_detection_size(), max_num=0, metric='default'
    )
    if bboxes.shape[0] == 0:
        return None
