...
ENGINE_FILE = "GPEN-BFR-512_fp16.engine"
    ...
    with THREAD_LOCK:
        if ENHANCER is None:
            ...
            if os.path.exists(engine_path):
                try:
                    print(f"{NAME}: Loading prebuilt TensorRT FP16 engine...")
                    ...
                    print(
                        f"{NAME}: Direct TensorRT engine failed: {error}. "
                        "Falling back to ONNX/CUDA."
                    )
                    ENHANCER = create_onnx_session(model_path)
                    warmup_session(ENHANCER)
            else:
                print(f"{NAME}: Prebuilt engine not found, using ONNX/CUDA.")
                ENHANCER = create_onnx_session(model_path)
                warmup_session(ENHANCER)
...
def enhance_face(temp_frame: Frame, face: Face) -> Frame:
    ...
    try:
        if session == "DIRECT_TRT_ENGINE":
            result, stats = enhance_face_trt(
                temp_frame, face, INPUT_SIZE
            )

            # Lightweight diagnostics every 100 GPEN calls.
            enhance_face._count = getattr(enhance_face, "_count", 0) + 1
            if enhance_face._count % 100 == 0:
                print(
                    "[GPEN TRT DETAIL] "
                    f"n={enhance_face._count} | "
                    f"infer={stats['infer']:.2f}ms | "
                    f"warp+blend={stats['warp_blend']:.2f}ms | "
